...
import { Goods, GoodsParams, Shop, getProductDetail, getRecommend } from '@/network/productDetail'
...
export default {
  ...
  methods: {
    ...
    titleItemClick(index) {
      // 根据数组下标滚动到对应的元素内容位置
      this.$nextTick(() => {
        const el = document.getElementsByClassName('detail-set-scroll')
    ...
    detailScroll(position) {
      const detailPosition = position ? -position.y : 0
...
<template>
  <div class="product-detail">
    <ProductDetailNavBar ref="detailNavBar" @titleItemClick="titleItemClick" />
    <Scroll ref="scroll" :probe-type="3" class="scroll-height" @backTopScroll="detailScroll">
      <DetailSwiper :swiper-list="topImages" class="detail-set-scroll" />
      <DetailBaseInfo :goods="goods" />
      <DetailShopInfo :shop="shops" />
      <DetailImagesInfo :images-info="detailsInfo" @imgLoad="imgLoad" />
      <DetailParamInfo :param-info="paramsInfo" class="detail-set-scroll" />
      <DetailCommentInfo :comment-info="commentInfo" class="detail-set-scroll" />
      <GoodsList :goods="recommendList" :is-recommend="true" class="detail-set-scroll" />
    </Scroll>
